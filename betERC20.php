...
function tokenFromWei(number) {
  return number.div(web3js.toBigNumber('1' + '0'.repeat(tokenDecimals)));
}

function tokenToWei(number) {
  var bigNum = web3js.toBigNumber(number);
  var decimals = web3js.toBigNumber('1' + '0'.repeat(tokenDecimals));
  return bigNum.times(decimals);
}
...
function placeBetWithAddress(left, name, address) {
  simplewager.minBet(function(error, result) {
    var minBet = tokenFromWei(result);
    var prompt = "You are about to place bet on " + name + ".\nThe minimum bet is " + minBet + " " + tokenSymbol + ".\nPlease enter your bet:";
    ...
    var amount = tokenToWei(bet);
    var contractAddr = "<?php echo $contract; ?>";
    if (confirm("Bet on " + name + " with " + bet + " " + tokenSymbol + "?")) {
      tokenerc20.approveAndCall(contractAddr, amount, left ? '0x01' : '0x00', function(error, result) {
        if (!error) {
...
function runApp() {
  checkNetwork(function() {
    document.getElementById('tokenname').innerHTML = tokenName + " (" + tokenSymbol + ")";
    ...
    simplewager.leftTotalBettingAmount(function(error, leftAmount) {
      document.getElementById('leftamount').innerHTML = tokenFromWei(leftAmount) + " " + tokenSymbol;
    });
    ...
    simplewager.rightTotalBettingAmount(function(error, rightAmount) {
      document.getElementById('rightamount').innerHTML = tokenFromWei(rightAmount) + " " + tokenSymbol;
    });
                          ...
                          if(!error) {
                            simplewager.totalBettingAmountMinusHouseCut(function(error, totalBettingAmountMinusHouseCut) {
                              if (!error) {
                                ...
                                document.getElementById('housecutamount').innerHTML = 'House Cut is ' + tokenFromWei(houseCutAmount) + ' ' + tokenSymbol + ' (fees may apply)';
                              }
                            });
                          }
                        });
                              ...
                              if (!error && !result.isZero()) {
                                simplewager.winningAlreadyClaimed(address, function(error, claimed) {
                                  if (!error && !claimed) {
                                    ...
                                    document.getElementById('leftwinning').innerHTML = 'Your estimated winning is ' + tokenFromWei(estimatedProceeding) + ' ' + tokenSymbol + ' (fees may apply)';
                                  }
                                });
                              }
                            });
                              ...
                              if (!error && !result.isZero()) {
                                simplewager.winningAlreadyClaimed(address, function(error, claimed) {
                                  if (!error && !claimed) {
                                    ...
                                    document.getElementById('rightwinning').innerHTML = 'Your estimated winning is ' + tokenFromWei(estimatedProceeding) + ' ' + tokenSymbol + ' (fees may apply)';
                                  }
                                });
                              }
                            });
...
window.addEventListener('load', function() {
  ...
  if (typeof web3 !== 'undefined') {
    ...
    simplewager.tokenAddress(function(error, result) {
      if (!error) {
        tokenAddress = result;
        tokenerc20 = web3js.eth.contract(tokenerc20ContractABI).at(tokenAddress);
        tokenerc20.name(function(error, name) {
          tokenName = name;
          tokenerc20.symbol(function(error, symbol) {
            tokenSymbol = symbol;
            tokenerc20.decimals(function(error, decimals) {
